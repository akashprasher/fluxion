!(function (global, undefined) {
	...
	function realpath(path) {
		for (path = path.replace(DOT_RE, "/"); path.match(DOUBLE_DOT_RE); )
	...
	function addBase(id, refUri) {
		...
		return 0 === ret.indexOf("//") && (ret = location.protocol + ret), ret;
	}
	...
	function addOnload(node, callback, isCSS, url) {
		function onload() {
			(node.onload = node.onerror = node.onreadystatechange = null),
				isCSS || data.debug || head.removeChild(node),
				(node = null),
				callback();
		}
		var supportOnload = "onload" in node;
		return !isCSS || (!isOldWebKit && supportOnload)
			? void (supportOnload
					? ((node.onload = onload),
					  (node.onerror = function () {
							emit("error", { uri: url, node: node }), onload();
					  }))
	...
	if (!global.seajs) {
		var seajs = (global.seajs = { version: "2.2.1" }),
			data = (seajs.data = {}),
		...
		(seajs.on = function (name, callback) {
			...
			(seajs.off = function (name, callback) {
				if (!name && !callback)
					return (events = data.events = {}), seajs;
		...
		(Module.prototype.resolve = function () {
			...
			(Module.prototype.load = function () {
				...
				if (!(mod.status >= STATUS.LOADING)) {
					...
					emit("load", uris, mod);
					for (
						var m, len = (mod._remain = uris.length), i = 0;
						len > i;
						i++
					)
						(m = Module.get(uris[i])),
			...
			(Module.prototype.onload = function () {
				...
				for (uri in waitings)
					waitings.hasOwnProperty(uri) &&
						((m = cachedMods[uri]),
						(m._remain -= waitings[uri]),
						0 === m._remain && m.onload());
			...
			(Module.prototype.fetch = function (requestCache) {
				...
				return !requestUri || fetchedList[requestUri]
					? void mod.load()
					...
					: ((fetchingList[requestUri] = !0),
					  ...
					  void (
							emitData.requested ||
							(requestCache
								? (requestCache[emitData.requestUri] =
										sendRequest)
								: sendRequest())
					  ));
			}),
			...
			(Module.get = function (uri, deps) {
				return (
					cachedMods[uri] || (cachedMods[uri] = new Module(uri, deps))
				);
			}),
